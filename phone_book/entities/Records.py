...
class Record:
    ...
    def __str__(self):
        birthday = f"Birthday: {Fore.MAGENTA}{self.birthday}{Fore.RESET}" if self.birthday else ''
        return  f"Contact name: {Fore.CYAN}{self.name._value.capitalize()}{Fore.RESET}. " \
                f"Phones: {Fore.YELLOW}{' '.join(p.value for p in self.phones)}{Fore.RESET} " + birthday